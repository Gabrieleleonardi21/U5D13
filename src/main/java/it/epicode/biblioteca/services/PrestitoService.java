...
import it.epicode.biblioteca.dto.*;
...
public class PrestitoService {
    ...
    public PrestitoResponse apri(NuovoPrestitoRequest r, UUID adminId) {
        ...
        Prestito prestito = crea(userRepository.getReferenceById(r.userId()),
                libroRepository.getReferenceById(r.libroId()), r.durata(), adminId);
    ...
    /**
     * Crea e salva il prestito SENZA toccare le copie: chi chiama le ha già scalate
     * (apri al banco con prendiCopia, oppure la richiesta del lettore che le ha prenotate).
     */
    Prestito crea(User user, Libro libro, DurataPrestito durata, UUID adminId) {
        DurataPrestito scelta = durata;
        if (scelta == null) {
            scelta = DurataPrestito.MEDIA;
        }
        int giorni = costanteService.intero(scelta.chiaveCostante());
