...
// Da React Bits (GlowCursor-JS-CSS). Modifiche locali, segnate con [MOD]:
// - followWindow: segue il puntatore su tutta la finestra (per l'overlay globale in CursorGlow)
// - niente render quando la scia è completamente sparita: la GPU resta ferma da cursore fermo
...
const GlowCursor = ({
  ...
  enabled = true,
  followWindow = false, // [MOD] ascolta il puntatore sulla finestra invece che sul contenitore
  children,
  className = '',
  style,
  ...rest
  ...
  // [MOD] Aggiornato dopo ogni render e non durante (regola react-hooks/refs); il loop legge sempre i valori recenti
  useLayoutEffect(() => {
    propsRef.current = {
      color,
      secondaryColor,
  ...
  useEffect(() => {
    ...
    const render = now => {
      ...
      // [MOD] Scia invisibile: si pulisce il canvas una volta e si salta lo shader a schermo intero
      const invisibile = fadeTarget === 0 && fade < 0.001;
      if (!invisibile || !wasInvisible) renderer.render({ scene: mesh });
      wasInvisible = invisibile;
      if (!destroyed) raf = requestAnimationFrame(render);
    };

    const resizeObserver = new ResizeObserver(resize);
    resizeObserver.observe(container);
    // [MOD] Con followWindow l'overlay ha pointer-events: none, quindi si ascolta la finestra
    let sorgenteEventi = container;
    let sorgenteUscita = container;
    if (followWindow) {
      sorgenteEventi = window;
      sorgenteUscita = document.documentElement;
    }
    sorgenteEventi.addEventListener('pointermove', updatePointer);
    sorgenteEventi.addEventListener('pointerdown', updatePointer);
    sorgenteUscita.addEventListener('pointerleave', onPointerLeave);
    resize();
    ...
    return () => {
      ...
      sorgenteEventi.removeEventListener('pointermove', updatePointer);
      sorgenteEventi.removeEventListener('pointerdown', updatePointer);
      sorgenteUscita.removeEventListener('pointerleave', onPointerLeave);
      mesh.geometry.remove();
      program.remove();
    };
  }, [maxDevicePixelRatio, followWindow]);
